...
namespace AirDirector.Controls
{
    public partial class PlayerControlVideo : UserControl
    {
        ...
        private void OnTrackEnded() { _isPlaying = false; _updateTimer.Stop(); _mixCheckTimer.Stop(); _blinkTimer.Stop(); if (_autoMode) OnMixReached(); else { if (_playlistQueue != null && _playlistQueue.GetItemCount() > 0) { _playlistQueue.RemoveItem(0); Log("[MANUAL] Track ended: rimosso elemento completato dalla coda"); } ClearPlayer(); TrackEndedInManualMode?.Invoke(this, EventArgs.Empty); } }
